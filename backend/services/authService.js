...
const authService = {
  register: async (req, res) => {
    ...
    auditRepo.log(AUDIT.USER_REGISTERED, {
      userId: user._id,
      resourceId: user._id,
      resourceType: "User",
      ...getMeta(req),
  ...
  login: async (req, res) => {
    ...
    auditRepo.log(AUDIT.USER_LOGIN, {
      userId: user._id,
      ...getMeta(req),
    });
  ...
  resetPassword: async (req, res) => {
    ...
    if (!token || !password)
      throw Errors.validation("Token and password are required");
    if (password.length < 6)
      throw Errors.validation("Password must be at least 6 characters");
  ...
  changePassword: async (req, res) => {
    ...
    if (!currentPassword || !newPassword)
      throw Errors.validation("Both passwords are required");
    if (newPassword.length < 6)
      throw Errors.validation("New password must be at least 6 characters");
    ...
    auditRepo.log(AUDIT.PASSWORD_CHANGED, {
      userId: user._id,
      ...getMeta(req),
    });
  },
  ...
  updateProfile: async (req) => {
    ...
    auditRepo.log(AUDIT.PROFILE_UPDATED, {
      userId: req.user._id,
